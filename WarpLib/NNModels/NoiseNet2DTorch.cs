    ...
    public class NoiseNet2DTorch : IDisposable
    {
                          ...
                          out float[] loss)
        {
            ...
            Helper.ForCPU(0, NDevices, NDevices, null, (i, threadID) =>
            {
                ...
                using (TorchTensor PredictionFT = PredictionDeconvPadMask.rfftn(new long[] { 2, 3 }))
                using (TorchTensor PredictionFTConv = PredictionFT.Mul(TensorCTF[i]))
                using (TorchTensor PredictionPad = PredictionFTConv.irfftn(new long[] { 2, 3 }))
                using (TorchTensor Prediction1 = PredictionPad.Slice(3, 128, 512 - 128, 1))
                using (TorchTensor Prediction2 = Prediction1.Slice(2, 128, 512 - 128, 1))
                using (TorchTensor PredictionLoss = Loss(Prediction2, TensorTarget[i]))
                {
