...
namespace Warp
{
    public class NoiseNet3DTorch : IDisposable
    {
        ...
        private float[] ResultLossConsist = new float[1];

        private bool IsDisposed = false;

        public NoiseNet3DTorch(int3 boxDimensions, int[] devices, int batchSize = 4)
        {
            Devices = devices;
            ...
            Helper.ForCPU(0, NDevices, NDevices, null, (i, threadID) =>
            {
                ...
                TensorMask[i] = Float32Tensor.Ones(new long[] { DeviceBatch, 1, BoxDimensions.Z * 2, BoxDimensions.Y * 2, BoxDimensions.X * 2 }, DeviceType.CUDA, DeviceID);
                {
                    ...
                    for (int b = 0; b < DeviceBatch; b++)
                        GPU.CopyHostToDevice(MaskData,
                                             new IntPtr((long)TensorMask[i].DataPtr() + b * MaskData.Length * sizeof(float)),
                                             MaskData.Length);
                }
                                ...
                                out float[] loss)
        {
            ...
            Helper.ForCPU(0, NDevices, NDevices, null, (i, threadID) =>
            {
                ...
                //TorchTensor DetachedPredictionDeconv;
                ...
                {
                    GPU.CopyDeviceToDevice(Prediction3.DataPtr(),
                                           ResultPredicted.GetDeviceSlice(i * DeviceBatch * BoxDimensions.Z, Intent.Write),
                                           DeviceBatch * (int)BoxDimensions.Elements());
                    ...
                    //DetachedPredictionDeconv = PredictionDeconv.Detach();
                }

                using (TorchTensor PredictionDeconvTwist = UNetModel[i].Forward(TensorTarget[i]))
                ...
                using (TorchTensor Prediction2 = Prediction1.Slice(3, Margin.Y, Margin.Y + BoxDimensions.Y, 1))
                using (TorchTensor Prediction3 = Prediction2.Slice(2, Margin.Z, Margin.Z + BoxDimensions.Z, 1))
                using (TorchTensor PredictionLoss = Loss(Prediction3, TensorSource[i]))
                //using (TorchTensor ConsistLoss = Loss(PredictionDeconvTwist, DetachedPredictionDeconv))
                //using (TorchScalar ConsistScale = TorchSharp.ScalarExtensionMethods.ToScalar(ConsistFactor))
                //using (TorchTensor ConsistLossScaled = ConsistLoss.Mul(ConsistScale))
                //using (TorchTensor OverallLoss = PredictionLoss.Add(ConsistLossScaled))
                {
                    if (i == 0)
                    {
                        GPU.CopyDeviceToHost(PredictionLoss.DataPtr(), ResultLossTwisted, 1);
                        //GPU.CopyDeviceToHost(ConsistLoss.DataPtr(), ResultLossConsist, 1);
                    }

                    PredictionLoss.Backward();
                }

                //DetachedPredictionDeconv.Dispose();
            }, null);
