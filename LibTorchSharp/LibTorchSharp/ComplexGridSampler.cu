...
#include "CustomOperatorsBackend.cuh"
//#include "ComplexGridSampler.cuh"
...
namespace at {
    namespace native {
        ...
        namespace MyOperator {
            ...
            inline int MY_CUDA_GET_BLOCKS(const int N) {
            ...
            static __forceinline__ __device__
                scalar_t complex_grid_sampler_unnormalize(scalar_t coord, int size, bool align_corners) {
                if (align_corners) {
            ...
            // complex_grid_sampler_unnormalize_set_grad works the same as complex_grid_sampler_unnormalize
            // except that it also returns the `d output / d input` via pointer argument
            // `grad_in`.
            // This is useful in the backward pass of complex_grid_sampler_.
            template <typename scalar_t>
            static __forceinline__ __device__
                scalar_t complex_grid_sampler_unnormalize_set_grad(scalar_t coord, int size,
            ...
            template <typename scalar_t>
            static __forceinline__ __device__
                scalar_t complex_grid_sampler_compute_source_index(
                    scalar_t coord,
                    int size,
                    GridSamplerPadding padding_mode,
                    bool align_corners) {
                coord = complex_grid_sampler_unnormalize(coord, size, align_corners);
            ...
            // complex_grid_sampler_compute_source_index_set_grad works similarly to
            // complex_grid_sampler_compute_source_index except that it also returns the
            // `d output / d input` via pointer argument `grad_in`.
            // This is useful in the backward pass of complex_grid_sampler_.
            template <typename scalar_t>
            static __forceinline__ __device__
                scalar_t complex_grid_sampler_compute_source_index_set_grad(
                    scalar_t coord,
                    int size,
                    GridSamplerPadding padding_mode,
                    bool align_corners,
                ...
                coord = complex_grid_sampler_unnormalize_set_grad(coord, size, align_corners, grad_in);
                if (padding_mode == GridSamplerPadding::Border) {
                    // clip coordinates to image borders
            ...
            template <typename scalar_t>
            __global__ void complex_grid_sampler_2d_kernel(
                const int nthreads,
                ...
                CUDA_KERNEL_LOOP_TYPE(index, nthreads, int) {
                    ...
                    ix = complex_grid_sampler_compute_source_index(ix, inp_W, padding_mode, align_corners);
                    iy = complex_grid_sampler_compute_source_index(iy, inp_H, padding_mode, align_corners);
            ...
            template <typename scalar_t>
            __global__ void complex_grid_sampler_3d_kernel(
                const int nthreads,
                TensorInfo<c10::complex<float>, int> input,
                ...
                CUDA_KERNEL_LOOP_TYPE(index, nthreads, int) {
                    ...
                    ix = complex_grid_sampler_compute_source_index(ix, inp_W, padding_mode, align_corners);
                    iy = complex_grid_sampler_compute_source_index(iy, inp_H, padding_mode, align_corners);
                    iz = complex_grid_sampler_compute_source_index(iz, inp_D, padding_mode, align_corners);
            ...
            template <typename scalar_t>
            __global__ void complex_grid_sampler_2d_backward_kernel(
                const int nthreads,
                ...
                CUDA_KERNEL_LOOP_TYPE(index, nthreads, int) {
                    ...
                    ix = complex_grid_sampler_compute_source_index_set_grad(ix, inp_W, padding_mode, align_corners, &gix_mult);
                    iy = complex_grid_sampler_compute_source_index_set_grad(iy, inp_H, padding_mode, align_corners, &giy_mult);
            ...
            template <typename scalar_t>
            __global__ void complex_grid_sampler_3d_backward_kernel(
                const int nthreads,
                TensorInfo<c10::complex<float>, int> grad_output,
                TensorInfo<c10::complex<float>, int> input,
                TensorInfo<scalar_t, int> grid,
                ...
                CUDA_KERNEL_LOOP_TYPE(index, nthreads, int) {
                    ...
                    ix = complex_grid_sampler_compute_source_index_set_grad(ix, inp_W, padding_mode, align_corners, &gix_mult);
                    iy = complex_grid_sampler_compute_source_index_set_grad(iy, inp_H, padding_mode, align_corners, &giy_mult);
                    iz = complex_grid_sampler_compute_source_index_set_grad(iz, inp_D, padding_mode, align_corners, &giz_mult);
            ...
            // No shape checking needed here. See # NOTE [ complex_grid_sampler_ Native Functions ].
            Tensor complex_grid_sampler_2d_cuda(const Tensor& input, const Tensor& grid,
                ...
                int count = N * H * W;
                if (count > 0) {
                    complex_grid_sampler_2d_kernel<float>
                        KERNEL_ARGS4(MY_CUDA_GET_BLOCKS(count), MY_CUDA_MAX_THREADS, 0, at::cuda::getCurrentCUDAStream() ) (
            ...
            Tensor complex_grid_sampler_3d_cuda(const Tensor& input, const Tensor& grid,
                ...
                int count = N * D * H * W;
                if (count > 0) {
                    complex_grid_sampler_3d_kernel<float>
                        KERNEL_ARGS4(MY_CUDA_GET_BLOCKS(count), MY_CUDA_MAX_THREADS, 0, at::cuda::getCurrentCUDAStream())
                         (
                            static_cast<int>(count),
                            getTensorInfo<c10::complex<float>, int>(input),
            ...
            std::tuple<Tensor, Tensor>
                ...
                int count = N * H * W;
                if (count > 0) {
                    complex_grid_sampler_2d_backward_kernel<float>
                        KERNEL_ARGS4(MY_CUDA_GET_BLOCKS(count), MY_CUDA_MAX_THREADS, 0, at::cuda::getCurrentCUDAStream()) (
                            static_cast<int>(count),
            ...
            std::tuple<Tensor, Tensor>
                ...
                if (count > 0) {
                    ...
                    complex_grid_sampler_3d_backward_kernel<float>
                        KERNEL_ARGS4(MY_CUDA_GET_BLOCKS(count), MY_CUDA_MAX_THREADS, 0, at::cuda::getCurrentCUDAStream()) (
                            static_cast<int>(count),
                            getTensorInfo<c10::complex<float>, int>(grad_output),
                            getTensorInfo<c10::complex<float>, int>(input),
