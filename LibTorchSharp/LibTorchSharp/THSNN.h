...
EXPORT_API(void)		THSNN_Module_to_cuda(const NNModule optimizer, const int device_id);
EXPORT_API(void)        THSNN_Module_save(const NNModule module, const char * location);
EXPORT_API(void)		THSNN_Module_loadparams(const NNModule module, const char* location, const int device_type, const int device_index);
...
EXPORT_API(NNModule) THSNN_BatchNorm2d_ctor(const int64_t num_features, NNAnyModule* outAsAnyModule);
...
EXPORT_API(NNModule) THSNN_LeakyReLU_ctor(double negativeSlope, bool inplace, NNAnyModule* outAsAnyModule);
...
EXPORT_API(Tensor) THSNN_cross_entropy(const Tensor input, const Tensor target, const Tensor weight);
...
EXPORT_API(Optimizer) THSNN_RMSprop_ctor(const Tensor* parameters, const int length, const double learnig_rate, const double weight_decay);
EXPORT_API(void) THSNN_RMSprop_setlearningrate(const Optimizer optimizer, const double learning_rate);
