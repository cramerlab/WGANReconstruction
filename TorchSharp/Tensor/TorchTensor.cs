...
namespace TorchSharp.Tensor
{
    public sealed class TorchTensor : IDisposable
    {
        ...
        /// <summary>
        ///  Returns a pointer to the data managed by this tensor.
        /// </summary>
        /// <returns></returns>
        public IntPtr DataPtr()
        {
            IntPtr Result = THSTensor_data(handle);
        ...
        [DllImport("LibTorchSharp")]
        private static extern void THSTensor_scatter_to_devices(IntPtr tensor, long dim, long[] devices, long ndevices, IntPtr[] scattered_tensors);

        public TorchTensor[] ScatterToDevices(long dim, long[] devices)
        {
            int NDevices = devices.Length;
            IntPtr[] ResultPtr = new IntPtr[NDevices];

            THSTensor_scatter_to_devices(handle, dim, devices, NDevices, ResultPtr);
            Torch.CheckForErrors();

            TorchTensor[] Result = new TorchTensor[NDevices];
            for (int i = 0; i < NDevices; i++)
                Result[i] = new TorchTensor(ResultPtr[i]);
        ...
        [DllImport("LibTorchSharp")]
        private static extern void THSTensor_backward(IntPtr handle, IntPtr gradient, bool retain_graph);

        public void Backward(TorchTensor gradient, bool retain_graph = false)
        {
            THSTensor_backward(handle, gradient.handle, retain_graph);
            Torch.CheckForErrors();
        }

        public void Backward(bool retain_graph = false)
        {
            THSTensor_backward(handle, IntPtr.Zero, retain_graph);
            Torch.CheckForErrors();
        }
        ...
        [DllImport("LibTorchSharp")]
        extern static IntPtr THSTensor_detach(IntPtr handle);

        public TorchTensor Detach()
        {
        ...
        [DllImport("LibTorchSharp")]
        private static extern IntPtr THSTensor_softmax(IntPtr tensor, long dim);

        public TorchTensor Softmax(long dim)
        {
            var res = THSTensor_softmax(handle, dim);
            if (res == IntPtr.Zero)
                Torch.CheckForErrors();
            return new TorchTensor(res);
        }
        ...
        {
            var res = THSTensor_rfftn(handle, dims, (long)dims.Length);
            if (res == IntPtr.Zero) { Torch.CheckForErrors(); }
            return new TorchTensor(res);
        }

        [DllImport("LibTorchSharp")]
        private static extern IntPtr THSTensor_irfftn(IntPtr tensor, long[] dims, long ndims);

        public TorchTensor irfftn(long[] dims)
        {
            var res = THSTensor_irfftn(handle, dims, (long)dims.Length);
            if (res == IntPtr.Zero) { Torch.CheckForErrors(); }
            return new TorchTensor(res);
        }
        ...
        {
            var res = THSTensor_pad(handle, pad, pad.Length);
            if (res == IntPtr.Zero) { Torch.CheckForErrors(); }
            return new TorchTensor(res);
        }
