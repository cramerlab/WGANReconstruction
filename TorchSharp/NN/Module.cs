...
namespace TorchSharp.NN
{
    public class Module : IDisposable
    {
        ...
        [DllImport("LibTorchSharp")]
        extern static void THSNN_Module_loadparams(HType handle, [MarshalAs(UnmanagedType.LPStr)] string location, int device_type, int device_index);

        public virtual void Load(String modelPath, DeviceType deviceType, int deviceIndex)
        {
            THSNN_Module_loadparams(handle, modelPath, (int)deviceType, deviceIndex);
        }
        ...
        [DllImport("LibTorchSharp")]
        private static extern void THSNN_Module_to_cuda(HType module, int device_id);

        public virtual void ToCuda(int device_id)
        {
            THSNN_Module_to_cuda(handle, device_id);
            Torch.CheckForErrors();
        }
        ...
        [DllImport("LibTorchSharp")]
        private static extern void THSNN_module_synchronize_with(Module.HType module, Module.HType replica, long device);

        public virtual void SynchronizeTo(Module replica, long device)
        {
            THSNN_module_synchronize_with(handle, replica.handle, device);
            Torch.CheckForErrors();
        }

        [DllImport("LibTorchSharp")]
        private static extern void THSNN_module_gather_grad(Module.HType module, Module.HType replica);

        public virtual void GatherGrad(Module replica)
        {
            THSNN_module_gather_grad(handle, replica.handle);
            Torch.CheckForErrors();
        }

        [DllImport("LibTorchSharp")]
        private static extern void THSNN_module_scale_grad(Module.HType module, float factor);

        public virtual void ScaleGrad(float factor)
        {
            THSNN_module_scale_grad(handle, factor);
            Torch.CheckForErrors();
        }
    }
